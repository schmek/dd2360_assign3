...
using tval = std::chrono::time_point<clock_type>;
void gettimeofday(tval* out, std::nullptr_t)
{
  *out = clock_type::now();
}
...
__global__ void gpu_grayscale(int width, int height, float* image, float* image_out)
{
  ...
  ///////////////////////////////////////////////////////////
  auto h = blockIdx.y * blockDim.y + threadIdx.y;
  auto w = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  auto offset_out = h * width;
  auto offset = offset_out * 3;
...
__global__ void gpu_gaussian(int width, int height, float* image, float* image_out)
{
  ...
  if (w < (width - 2) && h < (height - 2))
  {
    ...
    sh_block[offset_sh] = image[offset_t + w];
    //printf(" %d \n", offset_sh);
    __syncthreads();
    image_out[offset + (w + 1)] = applyFilter(&sh_block[offset_sh], BLOCK_SIZE_SH, gaussian, 3);
  }
}
...
__global__ void gpu_sobel(int width, int height, float* image, float* image_out)
{
  ...
  if (h < height - 2 && w < width - 2)
  {
    ...
    sh_block[offset_sh] = image[offset_t + w];
    __syncthreads();
    float gx = applyFilter(&sh_block[offset_sh], BLOCK_SIZE_SH, sobel_x, 3);
    float gy = applyFilter(&sh_block[offset_sh], BLOCK_SIZE_SH, sobel_y, 3);
